...
interface BookmarkItem { id: string; chunk: string; category: string | null; note: string; created_at: string }
      ...
      <UContainer class="flex-1 flex flex-col gap-4 py-6">
        ...
        <div v-if="uncategorized.length" class="space-y-2">
          ...
          <div v-for="bm in uncategorized" :key="bm.id" class="flex items-start justify-between gap-2 p-3 bg-default rounded-lg ring ring-default">
            <div>
              <p class="text-sm">{{ bm.chunk }}</p>
              <p v-if="bm.note" class="text-xs text-dimmed mt-1">{{ bm.note }}</p>
            </div>
            <UButton size="xs" variant="ghost" color="error" icon="i-lucide-trash" @click="deleteBookmark(bm.id)" />
          </div>
        </div>
